...
class EncoderBlock(nn.Module):
    ...
    def token_dropping(self, x, attn, keep_rate=None, tokens=None):
        ...
        if self.keep_rate < 1 and keep_rate < 1 or tokens is not None:  # double check the keep rate
            ...
            return index, idx, left_tokens

        return None, None, left_tokens

    def aggregate_tokens(self, x, index, idx, cls_attn, left_tokens):
        non_cls = x[:, 1:]
        x_others = torch.gather(non_cls, dim=1, index=compl.unsqueeze(-1).expand(-1, -1, C)) # (B, N-1-left_tokens, C)
